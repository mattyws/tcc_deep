...
from DeepLearning import learn as deeplearn
...
def get_models(sequence_length, embedding_dim):
    filter_sizes = (3, 8)
    ...
    # model.add(Convolution1D(filters=2,
    ...
    # models['convnet'] = model
    # models['convnet'].compile(loss="categorical_crossentropy", optimizer="adam", metrics=["accuracy"])
    # # # Build model
    ...
    model.add(Dense(7, activation='sigmoid'))
    models['recnet'] = model
    models['recnet'].compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
...
# create_directories(get_models(0, 0).keys())
...
for level in structure:
    ...
    for dataP in data_paths:
        ...
        x = np.zeros(shape=(numDocs, 300, 200), dtype='int32')
        ...
        # model = doc2vec.Doc2Vec.load("doc2vec_saved/"+pathname+".model")
        model = word2vec.Word2Vec.load("word2vec_model.model")
        doc2vecModel = doc2vec.Doc2Vec.load("doc2vec_model.model")
        data_table = [[[]]]
        # y = []
        data = LoadCorpus(dataP, tokenizer=tokenizer, stop_set=stop_set, stemmer=stemmer)
        count = dict()
        for helper, d in enumerate(data):
            # data_table.append([model.infer_vector(d.words), d.tags[0]])
            for helper2, w in enumerate(d.words):
                if helper2 >= 300:
                    break
                try:
                    x[helper, helper2] = model[w]
                except:
                    x[helper, helper2] = [0]*200
            # x[helper, 4] = doc2vecModel.infer_vector(d.words)
        ...
        # print(x)
        # y = np_utils.to_categorical(y, len(list(level.keys())))
    #
        kf = KFold(2, shuffle=True, random_state=None)
        ...
        for trainIndex, testIndex in kf.split(x):
            models = get_models(300, 200)
            print(" ============== Fold ", i, "============")
            for key in models.keys():
                # try:
                print("Training " + key)
                trainDocs, testDocs = x[trainIndex], x[testIndex]
                trainCats2, testCats2 = y[trainIndex], y[testIndex]

                trainCats = to_categorical(trainCats2, 7)
                testCats = to_categorical(testCats2, 7)

                models[key].fit(trainDocs, trainCats, epochs=500)

                scores = models[key].evaluate(testDocs, testCats, verbose=0)
                print("\n%s: %.2f%%" % (models[key].metrics_names[1], scores[1] * 100))

                test = models[key].predict_classes(testDocs, batch_size=len(testDocs))
                print("Accuracy 2: " + str(accuracy_score(testCats2, test)))
